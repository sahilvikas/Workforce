<template>
	<div class="cmo-approvals-tab">
		...
		<div class="tab-header">
			<div>
				...
				<p class="tab-subtitle">
					{{ isCmoView
						? 'Requisitions from your team — approve to send them on to Priyesh for final approval'
						: 'Admin view: every requisition currently waiting on a manager approval' }}
				</p>
			</div>
		</div>
		...
		<div v-if="!loading && pendingRequisitions.length === 0 && historyRequisitions.length === 0" class="empty-state">
			...
			<p>No requisitions need a manager decision right now.</p>
		</div>
		...
		<div v-if="pendingRequisitions.length > 0" class="section">
			<div class="section-header">
				...
				<span class="section-hint">Priyesh sees these only after you approve</span>
			</div>
				...
				<div v-for="r in pendingRequisitions" :key="r.name" class="req-card" :class="{ overdue: r.days_pending > 7 }">
					...
					<div class="req-card-actions">
						<button class="btn-approve" @click="openApproveDialog(r)">
							<span class="btn-icon">✓</span> Approve &amp; send to Priyesh
						</button>
		...
		<div v-if="historyRequisitions.length > 0" class="section">
			<div class="section-header">
				<h3>
					...
					{{ isCmoView ? 'Recent decisions' : 'Stopped at the manager stage' }}
				</h3>
		...
		<DetailPanel :visible="showPanel" :title="selectedReq ? selectedReq.title : ''" size="lg" @close="closePanel">
			<div v-if="detailData" class="detail-content">
				...
				<div class="detail-facts-grid">
					...
					<div class="fact-item" v-if="detailData.requisition.approving_manager_name">
						<div class="fact-label">Approving Manager</div>
				...
				<div v-if="detailData.requisition.business_justification" class="detail-section">
					<div class="section-title">Business Justification</div>
					<div class="section-body">{{ detailData.requisition.business_justification }}</div>
				</div>
				...
				<div v-if="detailData.requisition.leadership_comment" class="detail-section revision-history">
					<div class="section-title">Decision trail</div>
					<div class="section-body comment-trail">{{ detailData.requisition.leadership_comment }}</div>
				</div>
			...
			<template #actions>
				<template v-if="detailData && detailData.permissions && detailData.permissions.can_cmo_decide">
		...
		<!-- ==================== APPROVE DIALOG ==================== -->
		<Dialog :visible="showApproveDialog" title="Approve & Send to Priyesh" submitLabel="Approve & Send"
			...
			<div class="form-group full">
				...
				<textarea v-model="decisionComment" class="form-input form-textarea" rows="3"
					placeholder="Any context for Priyesh..."></textarea>
			</div>
			<p class="dialog-note approve-note">
				This moves to Priyesh for final approval. The position is not live until he approves and HR publishes it.
			</p>
		</Dialog>
		...
		<Dialog :visible="showRequestChangesDialog" title="Request Changes" submitLabel="Send Back for Revision"
			...
			<p class="dialog-intro">
				Send <strong>{{ actionReq && actionReq.title }}</strong> back to the requester with your feedback:
			</p>
			<div class="form-group full">
			...
			<p class="dialog-note request-note">
				The requester can edit and resubmit. It comes back to this queue, not straight to Priyesh.
			</p>
		</Dialog>
		...
		<Dialog :visible="showRejectDialog" title="Reject Requisition" submitLabel="Confirm Reject"
			...
			<p class="dialog-intro">
				Reject <strong>{{ actionReq && actionReq.title }}</strong>? This closes the requisition here.
			</p>
			<div class="form-group full">
			...
			<p class="dialog-note reject-note">
				The requisition stops at this stage — Priyesh will not see it. The requester is notified and would need to raise a new one.
			</p>
		</Dialog>
	</div>
</template>
...
export default {
	...
	computed: {
		// wf_get_requisitions returns role_view = 'cmo' for a real approving manager,
		// and 'hr_manager' for System Manager / WF HR Manager (admin view).
		isCmoView() {
			return this.roleView === 'cmo';
		},

		// awaiting_me is only ever set for role_view 'cmo' — the script explicitly
		// zeroes it for HR and System Manager, so it is not used in the admin view.
		pendingRequisitions() {
			return this.requisitions
				.filter(r => {
					if (r.status !== 'Pending CMO Approval') return false;
					if (this.isCmoView && !r.awaiting_me) return false;
					return true;
				})
		...
		historyRequisitions() {
			// CMO sees their team's full trail. Admin view shows only the stage-terminal
			// state, since everything else is already visible in the Approvals tab.
			const done = this.isCmoView
				? ['Pending Approval', 'Approved', 'Published', 'Needs Revision', 'Rejected by CMO', 'Rejected']
				: ['Rejected by CMO'];
			return this.requisitions
		...
		async submitDecision(decision) {
			...
			try {
				...
				const messages = {
					...
					'Request Changes': 'Sent back to the requester for revision',
					'Rejected': 'Requisition rejected at the manager stage'
				};
...
<style scoped>
.cmo-approvals-tab { padding-bottom: 40px; }
...
.wf-table { width: 100%; border-collapse: collapse; min-width: 600px; }
...
.comment-trail { white-space: pre-wrap; }
.highlight-section {
	background: #eff6ff;
