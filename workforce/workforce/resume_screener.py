...
def run_resume_screening():
    ...
    # "not screened" = ai_score is null/empty AND not already marked failed.
    ...
    for row in applicants:
        ...
        try:
            ...
            try:
                frappe.db.set_value("WF Applicant", row["name"],
...
def _screen_one(applicant_id, api_key, model):
    ...
    if not text or len(text.strip()) < 30:
        ...
        frappe.db.set_value("WF Applicant", applicant_id,
            {"screening_status": "Screening Failed",
    ...
    if app.job_opening and frappe.db.exists("WF Job Opening", app.job_opening):
        j = frappe.db.get_value("WF Job Opening", app.job_opening, ["job_title", "description"], as_dict=True)
        job_title = j.get("job_title") or ""
        job_desc = j.get("description") or ""
        for s in frappe.get_all("WF Required Skill",
    ...
    # ---- write ADVISORY fields only (no status change beyond clearing failure) ----
    frappe.db.set_value("WF Applicant", applicant_id, {
        "ai_score": score if score > 0 else 1,   # keep non-zero so it's marked "screened"
        "ai_grade": grade,
...
def _score_with_azure(api_key, model, job_title, skills_text, job_desc, resume_text):
    azure_url = (AZURE_RESOURCE + "/openai/deployments/" + str(model)
                 + "/chat/completions?api-version=" + AZURE_API_VERSION)

    system_msg = ("You are an expert technical recruiter. Score how well a candidate's "
                  "resume matches a job. Be objective and concise. Respond ONLY with a "
                  "valid JSON object, no markdown.")
    json_shape = ('{"score": <0-100 integer>, "grade": "<A/B/C/D>", '
                  '"matched_skills": ["..."], "missing_skills": ["..."], '
                  '"summary": "<2-3 sentence assessment>"}')
    user_text = ("JOB TITLE: " + job_title
    ...
    resp = frappe.make_post_request(azure_url, headers=headers, json=body)
    content = ""
